<%@page import="mess.model.*,mess.service.*"%>
		...
		<table style="width:100%">		
		  <tr>
			...
			for(CustomerModel m:list)
			{
				...
				String name1=cts.getTypeById(cid);
				%>
				<tr>
				<td>Member type:</td>
				<td><%=name1%></td>
				...
				<%
				int id1=m.getEid();
				AdvanceService ad=new AdvancServiceImpl();
				int adv=ad.getAdvanceById(id1);
				%>
				<tr>
				<td>Advance paid:</td>
				<td><%=adv%></td>
				...
				<tr>
			      <td colspan="2"><center><a href='myupd?id=<%=m.getEid()%>&name=<%=m.getName()%>&email=<%=m.getEmail()%>&contact=<%=m.getContact()%>'>Update</a></center></td>
			    </tr>
				<%
			}
				%>

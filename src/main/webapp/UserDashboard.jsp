<%@page import="mess.service.CustomerServiceImpl"%>
<%@page import="mess.service.CustomerService"%>
<%@page import="mess.model.*,java.util.*" %>
<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">
<title>User-Dashboard</title>
<link rel="stylesheet"
	href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css" />
<link
	href="https://cdn.jsdelivr.net/npm/bootstrap@5.2.3/dist/css/bootstrap.min.css"
	rel="stylesheet" />
	<style>
	a{
      text-decoration:none;
	}
	</style>
</head>
<body>
<%
   int id=Integer.parseInt(session.getAttribute("uid").toString());
   CustomerService cs=new CustomerServiceImpl();
   List<CustomerModel> list=cs.getProfileById(id);
   String name="";
   for(CustomerModel m:list)
   {
	   name=m.getName();
   }
   
  %>
     <div class="container-fluid">
     <div class="row flex-nowrap">
        <div class="col-auto col-md-3 col-xl-2 px-sm-2 px-3 bg-light text-dark">
          <div class="dropdown pb-2 pt-4 pl-5">  
              <a class="d-flex align-items-center text-black text-decoration-none dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">              
                        <img src="data:image/png;base64," alt="Emp" width="90" height="90" class="rounded-circle">
                        <span class="d-none d-sm-inline mx-1 fs-2">User</span> 
              </a> 
              <ul class="dropdown-menu dropdown-menu-dark text-small">
                  <li><a class="dropdown-item" href="logout">Sign out</a></li>
              </ul>  
           </div>
           <hr>
           <div>  
               <center><h2 style="color:black;font-family: cursive;">Welcome<br><%=name%></h2></center>
           </div>
            <hr>
            <div class="d-flex flex-column align-items-center align-items-sm-start px-3 text-white min-vh-100">           
                <ul class="nav nav-pills flex-column mb-sm-auto mb-0 ms-1 show align-items-center  align-items-sm-start">
                            <li class="w-100">
                                <a href="ViewProfile.jsp?id=<%=id%>" class="nav-link fs-5 text-black px-0"> <span class="d-none d-sm-inline">View Profile</span></a>
                            </li>
                            <li>
                                <a href="ViewPresentDay.jsp?id=<%=id%>" class="nav-link fs-5 text-black px-0"> <span class="d-none d-sm-inline">View Present Days</span></a>
                            </li>
                            <li>
                                <a href="ViewPDMWByUser.jsp?id=<%=id%>" class="nav-link fs-5 text-black px-0"> <span class="d-none d-sm-inline">View Present Days Monthwise</span></a>
                            </li>
                            <li>
                                <a href="ViewMenuCardByUser.jsp" class="nav-link fs-5 text-black px-0"> <span class="d-none d-sm-inline">View Menu-Card</span></a>
                            </li>
                            <li>
                                <a href="ViewBill.jsp?id=<%=id%>" class="nav-link fs-5 text-black px-0"> <span class="d-none d-sm-inline">View Bill</span></a>
                            </li>
                  </ul>
                
            </div>
            
        </div>
    
    <script
		src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.11.6/dist/umd/popper.min.js"></script>
	<script
		src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.3/dist/js/bootstrap.min.js"></script>
</body>
</html>